...
def create_table(sfdatabasename,sfschemaname,sftablename,loadtype):
    print(sfdatabasename,sfschemaname,sftablename)
...
def create_stage(sfdatabasename,sfschemaname,cloud_path):
    ...
    if r's3://' in cloud_path:
        ...
        cloud_code='AZ'
        storage_integration = r'AZURE_CONTAINER'
    ...
    if len(result)==1:
        ...
        print(cloud_path)
        cloud_path = cloud_path.replace("https","azure")
        print(cloud_path)
...
def mergecommand(job,uploadfilename):
    ...
    try:
        ...
        if load_type=='FULL':
            ...
            if scd_type==2:
                ...
                try:
                    ...
                    runstmnt=merstatement
                    returnstmnt=str(e)
        update_audit_ingest = f"""UPDATE DATAMIGRATION.DEMO_USER.AUDIT_TABLE SET INGESTION_COMPLETED = 'YES' WHERE TD_DATABASE_NAME='{tddbname}' AND TD_TABLE_NAME='{tdtablename}' AND EXPORT_FILENAME = '{uploadfilename}' AND S3_PATH = '{cloud_path}' ;"""
        ...
        return [returncode,runstmnt,result]
...
def auditupdate(job,extract_start_dttm,extract_end_dttm,batch_id,job_id):
    print("AACHUTHA")
    '''
    print(job,export_start_time)
    tddbname=job[0]
    ...
    BATCH_ID, JOB_ID, TD_DATABASE_NAME, TD_TABLE_NAME, SF_DATABASE_NAME, SF_SCHEMA_NAME, SF_TABLE_NAME, LOAD_TYPE, TPT_SCRIPT_NAME, TPT_SCRIPT_CONTENT, TPT_SCRIPT_LOG, TPT_EXPORT_CMD, TPT_EXPORT_LOG, S3_UPLOAD_CMD, S3_UPLOAD_LOG, CREATE_TABLE_LOG, CREATE_STAGE_NAME, CREATE_STAGE_LOG, COPY_COMMAND, COPY_COMMAND_LOG, MERGE_STATEMENT, MERGE_STATEMENT_LOG, PREV_EXTRACTSTARTDTTM, PREV_EXTRACTENDDTTM, EXTRACTSTARTDTTM, EXTRACTENDDTTM,S3_PATH,EXPORT_FILENAME,EXECUTION_MODE, TD_TABLE_COUNT, SF_TABLE_COUNT, JOB_START_TIME, JOB_END_TIME, JOB_DURATION, FINAL_STATUS,INGESTION_COMPLETED)
    SELECT BATCH_ID, JOB_ID, TD_DATABASE_NAME, TD_TABLE_NAME, SF_DATABASE_NAME, SF_SCHEMA_NAME, SF_TABLE_NAME, LOAD_TYPE, TPT_SCRIPT_NAME, TPT_SCRIPT_CONTENT, TPT_SCRIPT_LOG, TPT_EXPORT_CMD, TPT_EXPORT_LOG, S3_UPLOAD_CMD, S3_UPLOAD_LOG, CREATE_TABLE_LOG, CREATE_STAGE_NAME, CREATE_STAGE_LOG, COPY_COMMAND, COPY_COMMAND_LOG, MERGE_STATEMENT, MERGE_STATEMENT_LOG, NULL, NULL, '{extract_start_dttm}', '{extract_end_dttm}',S3_PATH,EXPORT_FILENAME,EXECUTION_MODE, TD_TABLE_COUNT, SF_TABLE_COUNT, JOB_START_TIME, JOB_END_TIME, JOB_DURATION, FINAL_STATUS,INGESTION_COMPLETED FROM DATAMIGRATION.DEMO_USER.LOG_TABLE WHERE BATCH_ID = {batch_id} AND JOB_ID = {job_id};"""
    else:
        extract_start_dttm = f'{extract_start_dttm}'
        extract_end_dttm = f'{extract_end_dttm}'
        audit_query = f"""INSERT INTO DATAMIGRATION.DEMO_USER.AUDIT_TABLE(
            BATCH_ID, JOB_ID, TD_DATABASE_NAME, TD_TABLE_NAME, SF_DATABASE_NAME, SF_SCHEMA_NAME, SF_TABLE_NAME, LOAD_TYPE, TPT_SCRIPT_NAME, TPT_SCRIPT_CONTENT, TPT_SCRIPT_LOG, TPT_EXPORT_CMD, TPT_EXPORT_LOG, S3_UPLOAD_CMD, S3_UPLOAD_LOG, CREATE_TABLE_LOG, CREATE_STAGE_NAME, CREATE_STAGE_LOG, COPY_COMMAND, COPY_COMMAND_LOG, MERGE_STATEMENT, MERGE_STATEMENT_LOG, PREV_EXTRACTSTARTDTTM, PREV_EXTRACTENDDTTM, EXTRACTSTARTDTTM, EXTRACTENDDTTM,S3_PATH,EXPORT_FILENAME,EXECUTION_MODE, TD_TABLE_COUNT, SF_TABLE_COUNT, JOB_START_TIME, JOB_END_TIME, JOB_DURATION, FINAL_STATUS,INGESTION_COMPLETED)
            SELECT BATCH_ID, JOB_ID, TD_DATABASE_NAME, TD_TABLE_NAME, SF_DATABASE_NAME, SF_SCHEMA_NAME, SF_TABLE_NAME, LOAD_TYPE, TPT_SCRIPT_NAME, TPT_SCRIPT_CONTENT, TPT_SCRIPT_LOG, TPT_EXPORT_CMD, TPT_EXPORT_LOG, S3_UPLOAD_CMD, S3_UPLOAD_LOG, CREATE_TABLE_LOG, CREATE_STAGE_NAME, CREATE_STAGE_LOG, COPY_COMMAND, COPY_COMMAND_LOG, MERGE_STATEMENT, MERGE_STATEMENT_LOG, NULL, NULL, '{extract_start_dttm}', '{extract_end_dttm}',S3_PATH,EXPORT_FILENAME,EXECUTION_MODE, TD_TABLE_COUNT, SF_TABLE_COUNT, JOB_START_TIME, JOB_END_TIME, JOB_DURATION, FINAL_STATUS,INGESTION_COMPLETED FROM DATAMIGRATION.DEMO_USER.LOG_TABLE WHERE BATCH_ID = {batch_id} AND JOB_ID = {job_id};"""
    #print(audit_query) 
    ...
    return [returncode,auditstmnt,returnstmnt]
'''
    audit_query = f"""INSERT INTO DATAMIGRATION.DEMO_USER.AUDIT_TABLE (TD_DATABASE_NAME,TD_TABLE_NAME,EXTRACTSTARTDTTM,EXTRACTENDDTTM) SELECT ()
